# 🩸 Hemobit

### Gestão e Distribuição Inteligente de Hemocomponentes

Projeto desenvolvido para o **Projeto Integrador III — ADS 2026.2**, da **CESAR School**, integrando conhecimentos e práticas das diferentes disciplinas do 3º semestre.

O Hemobit é uma aplicação web para auxiliar na **gestão de hemocomponentes**, permitindo o controle de profissionais, hospitais, estoque e solicitações.

---

## 🎯 Objetivo

Centralizar e facilitar o gerenciamento do fluxo de hemocomponentes, desde o cadastro e controle do estoque até a solicitação e acompanhamento dos pedidos realizados pelos hospitais.

---

## 🛠️ Tecnologias

* **Java 17+**
* **Spring Boot**
* **Spring Data JPA**
* **Banco de dados relacional**
* **HTML / CSS**
* **Thymeleaf**
* **Git / GitHub**
* **Jira**
* **Figma**

> O projeto segue as restrições e boas práticas técnicas definidas para o desenvolvimento das disciplinas envolvidas, incluindo a não utilização de mecanismos de geração automática de código boilerplate, como o **Lombok**.

---

## 👥 Equipe

| Integrante                        | E-mail CESAR School                             |
| --------------------------------- | ----------------------------------------------- |
| Breno Luiz de Lima Cruz           | [bllc@cesar.school](mailto:bllc@cesar.school)   |
| Davi Magno Campelo do Nascimento  | [dmcn@cesar.school](mailto:dmcn@cesar.school)   |
| Lauan Goncalves dos Santos        | [lgs5@cesar.school](mailto:lgs5@cesar.school)   |
| Letícia Gomes da Silva            | [lgs6@cesar.school](mailto:lgs6@cesar.school)   |
| Lucas Felipe Barreto Cavalcante   | [lfbc@cesar.school](mailto:lfbc@cesar.school)   |
| Lucas Filipe de Lima Segundo      | [lfls@cesar.school](mailto:lfls@cesar.school)   |
| Mateus Valerino Barros de Santana | [mvbs2@cesar.school](mailto:mvbs2@cesar.school) |

### Histórico da equipe

| Integrante      | E-mail | Entrada | Saída |
| --------------- | ------ | ------- | ----- |
| Nenhum registro | —      | —       | —     |

---

# 📚 Entregas

## 🟢 Entrega 01 — POO

**Data: 31/08/2026**

A Entrega 01 contempla a definição das **7 Histórias de Usuário**, seus respectivos cenários de aceitação em **BDD** e o desenvolvimento do protótipo **Lo-Fi** da aplicação.

### 📋 Histórias de Usuário

As histórias e seus cenários BDD estão disponíveis no documento:

📄 **[Histórias de Usuário e Cenários BDD](./docs/historias_de_usuario.md)**

| ID       | História                                          |
| -------- | ------------------------------------------------- |
| **HU01** | Cadastro e Gestão de Profissionais Solicitantes   |
| **HU02** | Cadastro e Manutenção de Hospitais Solicitantes   |
| **HU03** | Cadastro do Acervo de Hemocomponentes             |
| **HU04** | Registro e Entrada de Bolsas de Sangue no Estoque |
| **HU05** | Consulta e Listagem do Estoque do Hemocentro      |
| **HU06** | Criação de Solicitação de Hemocomponentes         |
| **HU07** | Atualização do Status da Solicitação              |

### 🎨 Protótipo Lo-Fi

Protótipo desenvolvido no Figma contemplando as histórias selecionadas para esta entrega.

🔗 **[Acessar protótipo no Figma](COLOCAR_LINK_FIGMA)**

### 🎥 Screencast

Apresentação do protótipo Lo-Fi e explicação das histórias contempladas na Entrega 01.

▶️ **[Assistir ao screencast no YouTube](COLOCAR_LINK_YOUTUBE)**

---

## 🟡 Entrega 02

Em desenvolvimento.

---

## 🟠 Entrega 03

Em desenvolvimento.

---

## 🔴 Entrega 04

Em desenvolvimento.

---

## 🔗 Links

* 🐙 [Repositório GitHub](https://github.com/brenocrruz/projeto-hemobit)
* 📋 [Jira](https://algs2.atlassian.net/jira/software/c/projects/PI3E4/boards/11/backlog)
* 🎨 [Figma](COLOCAR_LINK_FIGMA)
* 🎥 [YouTube](COLOCAR_LINK_YOUTUBE)
